...
function playClickSound() {
    const audio = document.getElementById('click-sound-audio');
    if (audio) {
        audio.currentTime = 0;
        audio.play().catch(e => console.log("Click sound deferred:", e));
    }
}

// ==========================================
// 3D HERO AUTHENTICATION TRANSITIONS
// ==========================================
window.openHeroRegister = function() {
    playClickSound();
    setTimeout(() => {
        showRegister();
    }, 150);
...
window.showHeroLanding = function() {
    hideAuthMsgs();
    const registerForm = document.getElementById('register-form');
    const loginForm = document.getElementById('login-form');
    const forgotForm = document.getElementById('forgot-form');
    const heroView = document.getElementById('auth-hero-view');
...
// ==========================================
// PROFESSIONAL NAVIGATION STATE MACHINE
// ==========================================
...
// ==========================================
// SECURE DYNAMIC STATUS COMPUTER
// ==========================================
...
window.openMatchDetails = function(matchId) {
    ...
    if (effStatus === 'live' || effStatus === 'ongoing') {
        window.location.href = `live-match.html?id=${encodeURIComponent(matchId)}`;
    } else if (effStatus === 'completed') {
        window.location.href = `result-match.html?id=${encodeURIComponent(matchId)}`;
    } else {
        window.location.href = `upcoming-match.html?id=${encodeURIComponent(matchId)}`;
    }
};
...
// ==========================================
// 12:00 AM ROLLOVERS & WINNING LOGIC
// ==========================================
...
onAuthStateChanged(auth, async (user) => {
    ...
    if (user) {
        ...
        hideAllSections(); 
        document.getElementById('fab-contact-btn').style.display = 'none'; 
        document.getElementById('auth-section').style.display = 'flex'; 
        document.getElementById('shimmer-screen').style.display = 'none'; 
        showHeroLanding();
        currentUserDoc = null; 
    }
});
...
window.submitWithdrawal = async function() {
    ...
    try {
        ...
        try {
            ...
            console.log("RTDB trigger deferred: ", rtdbErr);
        }
