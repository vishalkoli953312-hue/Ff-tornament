...
// ==========================================
// 1. FIREBASE CONFIGURATION (Aapki Spinner Project Keys)
// ==========================================
const spinnerConfig = {
...
// ==========================================
// SYSTEM FUNCTIONS (MODULAR LOGIC FOR SPINNER)
// ==========================================

// A. Withdrawal request trigger karna (Storage bachega - Overwrite system)
...
// B. Nayi Speech Script template list mein add karna
...
// C. Kisi script ko active (Tick mark) select karna
window.selectActiveScript = function(scriptId, scriptText) {
    set(ref(rtdb, "settings/activeScriptId"), scriptId);
    return set(ref(rtdb, "settings/script"), scriptText); // Active script text
}

// D. Kisi script ko list se delete karna
window.deleteScriptTemplate = function(scriptId) {
    return remove(ref(rtdb, `settings/scripts/${scriptId}`));
}

// E. Amount bolne ki language select karna (Hindi 'hi' ya English 'en')
window.setAmountLanguage = function(langType) {
    return set(ref(rtdb, "settings/amountLang"), langType); // "en" or "hi"
}

// F. Baki physical settings save karna (Speed, Pitch, Volume, Stream)
...
// ==========================================
// OPTIMIZED CONFETTI SYSTEM (ZERO-LAG ENGINE)
// ==========================================
...
window.sendOTP = async function(btn) {
    ...
    try { 
        ...
        // Dynamic template sending using native SDK
        await emailjs.send(
            ejsData.serviceId, 
            ejsData.templateId, 
    ...
    try {
        ...
        if(!promoSnap.empty) { 
            promoSnap.forEach(docSnap => { 
                let p = docSnap.data(); 
                // Each slide is generated inside its own full 3D card wrapper dynamically
                track.innerHTML += `
                    <div class="h-banner-box tactile-3d">
                        <a href="${p.targetUrl || '#'}" target="${p.targetUrl ? '_blank' : '_self'}" class="h-banner-slide" style="background-image:url('${p.imgUrl}')"></a>
                    </div>
                `; 
...
window.submitWithdrawal = async function() {
    ...
    try {
        ...
        // Trigger announcement / speech feedback in spinner project rtdb
        try {
            await window.triggerWithdraw(amt);
        } catch(rtdbErr) {
            console.log("RTDB Announcer trigger skipped or error: ", rtdbErr);
        }
    ...
    btn.innerHTML = "WITHDRAW"; btn.disabled = false; showLoader(false);
}
